...
const RestLog = require("../../../TFM_Backend/models/restLog");
const notifyUser = require("../../../TFM_Backend/utils/notifyUser");
const {
  validateTrip,
} = require("../../../TFM_Backend/validationModels/validateTrip");
const tripController = require("../../../TFM_Backend/controllers/tripController");
...
describe("tripController", () => {
  describe("createTrip", () => {
    ...
    it("should create trip and notify users", async () => {
      ...
      expect(notifyUser).toHaveBeenCalledWith(
        "owner123",
        expect.stringContaining("Trip created successfully")
      );
      expect(res.status).toHaveBeenCalledWith(201);
      expect(res.send).toHaveBeenCalledWith(
        expect.objectContaining({ truck_id: "truck1", driver_id: "driver123" })
      );
    });
  });
  ...
  describe("getTripById", () => {
    ...
    it("should return trip with remaining km", async () => {
      ...
      await tripController.getTripById(req, res);
      expect(res.send).toHaveBeenCalledWith(
        expect.objectContaining({ remaining_km_in_trip: 50 })
      );
    });
  });
  ...
  describe("startTrip", () => {
    ...
    it("should start trip, create session, and notify", async () => {
      ...
      expect(notifyUser).toHaveBeenCalledWith(
        "driver123",
        expect.stringContaining("Your trip has started")
      );
      expect(notifyUser).toHaveBeenCalledWith(
        "owner123",
        expect.stringContaining("has started the trip")
      );
      expect(res.send).toHaveBeenCalledWith(
        expect.objectContaining({ message: "Trip started", trip, firstSession })
      );
    });
  });
  ...
  describe("completeTrip", () => {
    it("should return 400 if fuel_left missing", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({
        message: "fuel_left is required to complete the trip",
      });
    });
    it("should return 404 if trip not found", async () => {
    ...
    it("should complete trip, close sessions, and notify", async () => {
      ...
      expect(notifyUser).toHaveBeenCalledWith(
        "driver123",
        expect.stringContaining("completed the trip successfully")
      );
      expect(notifyUser).toHaveBeenCalledWith(
        "owner123",
        expect.stringContaining("has completed the trip")
      );
      expect(res.json).toHaveBeenCalledWith(
  ...
  describe("updateTrip", () => {
    ...
    it("should update allowed fields and return success", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ message: "Trip updated successfully", trip })
      );
    });
    it("should handle errors and return 500", async () => {
      req.params.id = "t1";
  ...
  describe("getAllTrips", () => {
    it("should return 403 for driver role", async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(403);
      expect(res.send).toHaveBeenCalledWith(
        "Drivers are not allowed to view all trips."
      );
    });
  ...
  describe("getMyTrips", () => {
    it("should return 403 if user not driver", async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(403);
      expect(res.send).toHaveBeenCalledWith(
        "Only drivers can view their trips."
      );
    });
