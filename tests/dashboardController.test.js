...
describe("Dashboard Controller", () => {
  ...
  describe("getStats", () => {
    test("returns correct stats for owner", async () => {
      ...
      expect(Truck.countDocuments).toHaveBeenCalledWith({ owner_id: "user123" });
      expect(Trip.countDocuments).toHaveBeenCalledWith({ owner_id: "user123", isDeleted: false });
      expect(User.countDocuments).toHaveBeenCalledWith({ role: "driver", ownedBy: "user123" });
    ...
    test("returns correct stats for driver", async () => {
      ...
      expect(Trip.countDocuments).toHaveBeenCalledWith({ driver_id: "user123", isDeleted: false });
  ...
  describe("getRecentTrips", () => {
    ...
    test("returns recent trips for owner", async () => {
      ...
      expect(Trip.find).toHaveBeenCalledWith({ owner_id: "user123", isDeleted: false });
    ...
    test("returns recent trips for driver", async () => {
      ...
      expect(Trip.find).toHaveBeenCalledWith({ driver_id: "user123", isDeleted: false });
      expect(res.json).toHaveBeenCalledWith(sampleTrips);
    });
  ...
  describe("getRecentDriveSessions", () => {
    ...
    test("returns recent drive sessions for owner", async () => {
      ...
      expect(DriveSession.find).toHaveBeenCalledWith({ trip_id: { $in: ["trip1", "trip2"] } });
    ...
    test("returns recent drive sessions for driver", async () => {
      ...
      expect(DriveSession.find).toHaveBeenCalledWith({ trip_id: { $in: ["trip3", "trip4"] } });
    });
