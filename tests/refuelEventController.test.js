...
const refuelEventController = require('../controllers/refuelEventController');
...
describe('refuelEventController', () => {
  ...
  describe('logRefuel', () => {
    it('should return 400 if validation fails', async () => {
      validateRefuelEvent.mockReturnValue({ error: { details: [{ message: 'Invalid data' }] } });
    ...
    it('should log refuel and notify users', async () => {
      ...
      const trip = { driver_id: 'user123', owner_id: 'owner123', driver_snapshot: { name: 'Test Driver' } };
      Trip.findById.mockResolvedValue(trip);
      ...
      expect(notifyUser).toHaveBeenCalledWith('user123', expect.stringContaining('Refuel log added successfully'));
      expect(notifyUser).toHaveBeenCalledWith('owner123', expect.stringContaining('logged a refuel'));
